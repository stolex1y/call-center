...
std::chrono::year_month_day ExtractDate(const std::chrono::sys_time<Duration> &time_point) {
  return std::chrono::year_month_day{std::chrono::floor<std::chrono::days>(time_point)};
}

template<typename Duration>
std::chrono::year_month_day ExtractDate(const std::chrono::utc_time<Duration> &time_point) {
  return std::chrono::year_month_day{std::chrono::floor<std::chrono::days>(time_point)};
}

template<typename BoostDuration, typename StdDuration>
BoostDuration CastToBoost(const StdDuration &duration) {
  auto ms = boost::chrono::microseconds{std::chrono::duration_cast<std::chrono::microseconds>(duration)};
...
BoostTimePoint CastToBoostTimePoint(const StdTimePoint &time_point) {
  ...
  auto duration_us = CastToBoost<boost::chrono::microseconds>(
      std::chrono::time_point_cast<std::chrono::microseconds>(time_point).time_since_epoch());
  return boost::chrono::time_point_cast<BoostDuration, BoostClock>(
      boost::chrono::time_point<BoostClock, boost::chrono::microseconds>{duration_us});
}
